    ...
    return (
        <div className={styles.pageContainer}>
            <ul className={styles.pagesList} >
                {(currentPage > 1)&&<li className={ styles.itemList} onClick={currentPage > 1 ? ()=>handleClick(currentPage-1):null}>&lt;</li>}
                ...
                { (currentPage < (pageNumbers.length)) && <li className={ styles.itemList} onClick={currentPage < (pageNumbers.length) ? ()=>handleClick(currentPage+1):null}>&gt;</li>} 
            </ul>
        </div>
    )
}